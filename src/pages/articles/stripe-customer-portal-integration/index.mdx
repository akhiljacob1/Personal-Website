...
- This tutorial shows how to start a basic app and implement stripe. If you're integrating the customer portal into an existing app, you can skip to [Customer portal integration](#customer-portal-integration).
...
Oh also, you need to create a Stripe account at https://stripe.com. Once you're in, navigate to https://dashboard.stripe.com/account/apikeys and grab your secret key. We'll keep this safe in your credentials file 🔐
...
Then add Stripe fields to your user model as below. The `stripe_customer_id` will hold the the unique customer id of the user on Stripe. The `subscription_status` column will hold information about whether the subscription is active, trialing or cancelled etc. The `plan` column will hold the which of the products they are currently subscribed to. Here are the commands you'll need.

_Note: Don't forget to make the plan column an integer type, because we'll be using it as an enum later on._
...
Okay it's time to listen to webhooks from Stripe. To do this, we need to find some headphones 🎧. Just kidding. Run the following command in a separate terminal window:
...
Now when you go to the Customer Portal and update subscription, the change will be in reflected in your database. And just like that you've integrated Stripe customer portal into a Rails app! Be proud! 😎
